...
export const patientInput = z.object({
  ...
  // Quick task 20260812 — mrn auto-generated by the repo. Renderer
  // never sends it; any stray mrn in patientInput is dropped by zod.
...
export const patientPatchInput = z
  .object({
    ...
    gender: z.enum(['male', 'female', 'other']).nullish(),
    // Quick task 20260812 — mrn removed from patch. .strict() rejects
    // any stray `mrn` field in the patch with a 422.
    phone: z.string().min(1).max(30).nullish(),
    notes: z.string().min(1).max(2000).nullish(),
  })
  .strict();
