...
describe('patients:create', () => {
  it('creates a row with all 6 fields and writes audit_log row in the same transaction', async () => {
    ...
    // Quick task 20260812 — mrn is auto-generated by the repo; renderer
    // never sends it. Assert the serial shape, not a specific value.
    expect(row.mrn).toMatch(/^MRN-\d{6}$/);
  ...
  // Quick task 20260812 — duplicate-MRN collision no longer exists; mrn is
  // auto-generated by the repo (nextMrn) inside the create transaction, so
  // every successful create gets a unique MRN. The `patientPatchInput` is
  // `.strict()`, so any stray `mrn` in a patch surfaces as IPC_VALIDATION —
  // that contract is covered by `tests/main/patients/update.test.ts` if
  // it exists, otherwise by `patients.test.ts` repo tests.
