...
describe('patients:list search + pagination + audit', () => {
  ...
  it('mrn filter returns the single matching row with metadata.mrn echo', async () => {
    await bootstrapAndLogin();
    ...
    // time. We create two patients, read their assigned MRNs back, then
    // search by the first one's MRN to verify the filter still works.
    const aliceRow = createPatient({ fullName: 'Alice', dob: '1990-01-01', gender: null, phone: null, notes: null });
    createPatient({ fullName: 'Bob', dob: '1990-01-01', gender: null, phone: null, notes: null });
    const alice = getPatient(aliceRow.id);
    expect(alice?.mrn).toMatch(/^MRN-\d{6}$/);

    const { listPatients } = await import('../../../src/main/ipc/patients');
    const { closeDb, getDb } = await import('../../../src/main/db');

    const result = listPatients({ mrn: alice!.mrn });
    expect(result.total).toBe(1);
    expect(result.rows.length).toBe(1);
    expect(result.rows[0].mrn).toBe(alice!.mrn);
    ...
    expect(auditMeta.metadata).not.toBeNull();
    const parsed = JSON.parse(auditMeta.metadata!);
    expect(parsed.mrn).toBe(alice!.mrn);
