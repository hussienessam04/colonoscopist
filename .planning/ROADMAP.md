...
**Plans:** 19 plans (6 base + 13 gap-closure / feature) — complete
...
- [x] 08-11-PLAN.md — Gap closure (G-08-5): same safeInvoke + EmptyStateCard pattern extended to `src/renderer/src/pages/SettingsCapture.tsx` + `src/renderer/src/pages/ProcedureRoom.tsx` (both use `window.api.capture.getDefaultDevice()` / `getPreset()` / `setDefaultDevice()` / `setPreset()` and crashed when the gate returned `{ok:false}` because Plan 08-10 didn't include them). All 4 capture.* IPC sites in each file wrapped; EmptyStateCard renders for the SettingsCapture page; ProcedureRoom reuses the existing no-device overlay path (Wave 5, depends_on: 08-10)
...
- [x] 08-13-PLAN.md — UI audit fixes (3 blockers + 9 warnings from `08-UI-REVIEW.md`): License.tsx clipboard catch uses new `machineIdCopyFailed` key (Blk 1); trial days use i18next ICU plurals — EN `_one`/`_other`, AR `_zero`/`_one`/`_two`/`_few`/`_many`/`_other` (Blk 2); SettingsSidebar License entry renders status dot + label via `useLicenseStatus` + lucide icon for colorblind a11y (Blk 3); EmptyStateCard uses shadcn `<Button>` variant; theme tokens (`border-border` instead of hardcoded `border-slate-200`); non-italic loading state; zero-state copy for trial ends today; `bg-slate-500` (unactivated dot) instead of muted `bg-slate-400`; "Start trial" copy polish; 9 new EN keys + ICU plural sets + 9 new AR keys (hand-written per Phase 7 D-20). **Deviation:** `tests/renderer/i18n/parity.test.ts` relaxed to accept AR-only CLDR plural suffixes (forward parity untouched). (Wave 5, depends_on: 08-12)
  - **Verification:** `node scripts/run-vitest.cjs --run tests/renderer/pages/license.test.tsx tests/renderer/pages/license-gate.test.tsx` green; `node scripts/run-vitest.cjs --run tests/renderer/i18n/parity.test.ts` green with relaxed plural rule; 3 atomic commits.
- [x] 08-14-PLAN.md — Feature: Screenshot Crop (user-requested after Phase 8 UAT). New `IPC.SCREENSHOTS_CROP` channel added to `EXEMPT_CHANNELS` (crop is a routine clinical action, not gated). Main-side `src/main/screenshots/crop.ts` validates dimensions + crop-rect bounds + screenshot existence → `fs.writeFileSync` overwrites the source JPEG → emits `screenshot.cropped` audit row with `{ originalWidth, originalHeight, cropRect: {x,y,w,h}, newWidth, newHeight, byteSize }`. Renderer-side `<ScreenshotCropModal>` (shadcn Dialog + canvas + mouse-drag selection rectangle, no new native deps) wired into `<ScreenshotLightbox>` via a new Crop button next to the existing expand icon. Cache-bust refresh after successful crop. **Deviation:** crop bytes ride as base64 (`jpegBase64`) not raw `Uint8Array` — `z.instanceof(Uint8Array)` is unreliable across contextBridge; every other binary payload here uses base64. `id` is `number` to match existing screenshots surface. (Wave 5, depends_on: 08-13)
  - **Verification:** `node scripts/run-vitest.cjs --run tests/main/screenshots/crop.test.ts tests/renderer/components/screenshot-crop-modal.test.tsx` green; 3 atomic commits.
- [x] 08-15-PLAN.md — Gap closure (G-08-8): 4 user-reported fixes from Phase 8 UAT iteration. **(1)** Crop canvas taint: new `IPC.SCREENSHOTS_GET_BLOB` channel returns the screenshot bytes; renderer creates `URL.createObjectURL(new Blob(...))` → loads `<img>` from same-origin blob URL → no canvas taint. **(2)** Free-form polygon crop: ScreenshotCropModal refactored from rectangle-drag to click-to-add polygon vertices (min 3, double-click to finalize, Esc cancel, Backspace undo last). SVG `<polygon>` overlay renders the free-form shape. Main-side `cropScreenshot` accepts `cropPolygon: {x,y}[]` + computes the polygon's axis-aligned bounding box + crops the bbox region (v1 simplification; per-pixel polygon masking is a v1.1 follow-up). **(3)** Reports column restoration: new migration `0012_revert_report_redesign_drop_columns.sql` re-adds `findings` / `diagnosis` / `recommendations` / `procedure_details` columns that the untracked `redesign-report-procedure-type` migration 0010 had dropped from the user's DB. Registered in `src/main/db/migrations.ts`. **Deviation:** migration filename `0012` (not `0011` per plan) because `0011_settings_trial_started_at.sql` already exists. **(4)** Clipboard copy in Electron sandbox: new `IPC.CLIPBOARD_COPY_TEXT` channel uses Electron's main-process `clipboard.writeText` via `import('electron')` (avoids the unreliable `navigator.clipboard.writeText` in sandboxed contexts). Added to `EXEMPT_CHANNELS`. License.tsx `handleCopyMachineId` now calls `window.api.clipboard.copyText({text})`. (Wave 5, depends_on: 08-14)
  - **Verification:** `node scripts/run-vitest.cjs --run tests/main/screenshots/crop.test.ts tests/renderer/components/screenshot-crop-modal.test.tsx tests/renderer/pages/license.test.tsx` green (5 + 11 + 9 = 25 tests); `tests/main/db/migrations/0012_revert_report_redesign_drop_columns.test.ts` 3/3 pass (new regression test prevents future re-drops); `node scripts/check-license-gate.cjs` OK with 16 EXEMPT_CHANNELS; 3 atomic commits.
- [x] 08-16-PLAN.md — Gap closure (G-08-9): easier crop dot placement via mode toggle (Rectangle | Free-hand | Polygon). ScreenshotCropModal gets a `mode: 'rectangle' | 'freehand' | 'polygon'` state (default rectangle — simplest UX). Rectangle mode: mousedown + mousemove + mouseup creates a 4-corner polygon from the drag rect (one-shot). Free-hand mode: mousedown + mousemove samples path points every 5px; mouseup closes the polygon. Polygon mode (existing) stays for precise manual vertex placement. Mode toggle UI = 3 shadcn `<Button variant>` buttons at the top of the modal. All modes feed the existing polygon→bbox crop pipeline. 4 new i18n keys per language. **Auto-fix surfaced during verification:** mouseup was reading the release point from React state (lags one event under React 18 batching); fix reads `pointFromEvent(e)` off the mouseup event directly. (Wave 5, depends_on: 08-15)
  - **Verification:** `tests/renderer/components/screenshot-crop-modal.test.tsx` 18/18 pass (12 prior + 6 new covering Rectangle commit, Free-hand sampling, mode switching, Polygon regression); full renderer Vitest 341/341 pass; typecheck:web clean for modified file.
- [x] 08-17-PLAN.md — Gap closure (G-08-10): shape editing after commit (vertex drag + shape move + Clear) + live preview after crop. ScreenshotCropModal gets `draggingVertex` / `draggingShape` / `dragStart` state. After Rectangle/Free-hand/Polygon commit the shape stays editable: drag a vertex dot → only that vertex moves (resize); drag inside the shape → all vertices translate together (move); click Clear button → shape resets + Apply disables. Cursor flips: `crosshair` on commit-empty → `move` on committed → `grabbing` during drag. ScreenshotLightbox now uses SCREENSHOTS_GET_BLOB IPC + `URL.createObjectURL` → `<img src={blobUrl}>`; `cacheBuster` state bumped in `onCroped` callback → image refreshes immediately after crop (no leave/re-enter required). 2 new i18n keys per language. (Wave 5, depends_on: 08-16)
  - **Verification:** `tests/renderer/components/screenshot-crop-modal.test.tsx` 25/25 pass (12 prior + 13 new covering vertex drag, shape move, Clear button, cursor states, mode-switch-clears-polygon); `tests/renderer/components/screenshot-lightbox.test.tsx` 5/5 pass; full renderer/components suite 98/98 pass; ProcedureReview page tests 13/13 (no regression from blob-URL pattern).
- [x] 08-18-PLAN.md — Gap closure (G-08-11): crop modal diagnostic errors + Retry + cacheBuster refresh + media-server URL fallback. ScreenshotCropModal: `imgErrorDetail` carries the actual IPC code (e.g. `IPC_SCREENSHOT_NOT_FOUND`) instead of generic "Crop failed" — surfaces why getBlob fails. `retry()` button bumps `retryNonce` to re-trigger the fetch. `cacheBuster` prop re-fetches blob URL on change (so the modal image updates immediately after crop, not just the lightbox). `fallbackSrc` prop renders a read-only `<img src={mediaServerUrl}>` when getBlob fails — defensive fallback so the user can at least see the screenshot even if the IPC fails. ScreenshotLightbox composes `fallbackSrc` via the existing `screenshotUrl()` helper + passes `cacheBuster` + `fallbackSrc` to the modal. 2 new i18n keys per language. (Wave 5, depends_on: 08-17)
  - **Verification:** `tests/renderer/components/screenshot-crop-modal.test.tsx` 29/29 pass (25 prior + 4 new covering diagnostic error UI, Retry re-fetch, cacheBuster re-fetch, catch path message); `tests/renderer/components/screenshot-lightbox.test.tsx` 5/5 pass; full renderer/components suite 102/102 pass.
- [x] 08-19-PLAN.md — Gap closure (G-08-12): "browser rejected blob" error fix. **Root cause:** main-side returned `new Uint8Array(buffer)` where `buffer` is the `Buffer` from `fs.promises.readFile` — the Buffer-backed Uint8Array view doesn't survive Electron IPC structured clone cleanly; the browser rejects the resulting blob. **Fix:** change IPC contract type from `Uint8Array` to `ArrayBuffer`; main now slices + copies the Buffer into a fresh standalone `ArrayBuffer`; renderer wraps the received `ArrayBuffer` in `new Uint8Array(buffer)` for `Blob` construction. `ScreenshotCropModal` + `ScreenshotLightbox` both updated. (Wave 5, depends_on: 08-18)
  - **Verification:** `tests/main/screenshots/get-blob.test.ts` (new) 3/3 pass (happy path: ArrayBuffer + JPEG mime; not-found: IPC_SCREENSHOT_NOT_FOUND; corrupted file); `tests/renderer/components/screenshot-crop-modal.test.tsx` 30/30 pass (mock updated to ArrayBuffer); `tests/renderer/components/screenshot-lightbox.test.tsx` 5/5 pass; total 43/43 affected tests green.
