...
**Last Activity Description:** Phase 08 marked complete
...
| 2026-09-06 | timeline-thumbnails-refresh-after-crop | After crop, lightbox preview updated immediately but timeline thumbnail kept showing pre-crop image until leave/re-enter. Added shared `mediaCacheBuster` (ProcedureReview bumps on every crop via lightbox `onCropped`); timeline appends `?v=<bust>` to each MediaServer URL so the browser refetches. +1 contract-guard test. ScreenshotTimeline tests 15/15 pass. |
| 2026-09-06 | patient-procedures-duplicate-row-and-open-pdf-gate | (1) Each recording produced TWO procedure rows: `procedures.create` IPC creates row A (recording/empty path); recorder's `proceduresRepo.insert({id: A,...})` SILENTLY ignored input.id and minted a new UUID for row B — finalize pass updated row A so A ended up completed/empty, B stayed recording/with-mp4. Repo now honors input.id: existing id → updateVideoPath (preserves original via COALESCE); no id → fresh UUID as before. (2) Open PDF button gated only on `report !== null` (true for draft rows with pdfPath=null → opens non-existent file). Gate now also checks `report.pdfPath !== null`. +3 repo + 1 renderer test. |
| 2026-09-06 | finish-report-redesign-integration | Recovery: the 20260812 redesign work was half-done on disk but never integrated. IPC handler REPORTS_SET_PROCEDURE_TYPE crashed (called undefined repo method); ReportEditor rendered old Findings/Diagnosis textareas that always showed empty; PDF never generated. Committed the pending artifacts (migration, repos, dialogs, hook, pdf-locked-error helper), added setProcedureType/setInstrument/setPremedicationOverride to reportsRepo, rewrote ReportEditor with procedure-type toggle + 8 boxes + instrument picker + premedication override + templates picker, updated PDF render to the new shape, added i18n keys (EN + AR), updated tests for the new fields. |
| 2026-09-06 | report-editor-polish | (1) Added the 8 missing `common.*` i18n keys (`patient`, `mrn`, `dob`, `gender`, `procedure`, `date`, `duration`, `doctor`, `name`) — the editor was rendering raw `common.patient` etc. instead of labels. (2) Procedure-type toggle is now free at any time (dropped `boxesLocked` + repo SQL invariant). (3) Inputs (textareas, instrument, premedication, templates buttons) editable post-finalize. (4) Dropped the logo + signature header band from the editor preview (kept on the rendered PDF). (5) Anatomy boxes in a 2-column grid for better screen fit. (6) PDF header/footer image rendering is unchanged — already wired via the orchestrator + template; the bands render when the assets are uploaded in Settings → Profile. |
| 2026-09-06 | report-editor-layout-and-bullets | (1) Verified `procedure_type is locked` is gone from source (stale-build artifact; needs Electron restart). (2) Editor widened from `max-w-3xl` to `max-w-7xl`. (3) Editor document restructured into a 2-column grid: patient/procedure + 8 boxes on the left, ScreenshotTimeline as a sticky right rail on `lg+` screens. (4) Per-box bullet toggle (• prefix every non-empty line; idempotent). +1 contract test. |
| 2026-09-06 | report-editor-procedure-layout-screenshots-grid-bullet-button | (1) Procedure meta row — Date / Duration / Doctor now on a single row via grid-cols-3 with small-caps labels. (2) `formatHHMMSS` was timezone-dependent (`new Date(ms).getHours()` drifted by UTC offset); replaced with pure-math `formatDuration(seconds)`. (3) Bullet button moved from the heading row into the box itself (absolute bottom-right of the textarea + hint line below). (4) `ScreenshotTimeline` now accepts `layout='row'|'grid'`; the report editor's right rail passes `layout="grid"` (2-column vertical grid, overflow-y auto). +1 contract test. |
| 2026-09-06 | report-editor-clinical-refresh | (1) Dropped the bullet-hint `<p>` under each textarea (the button keeps the hint on hover via `title`). (2) Instrument + Pre-medication wrapped in a 2-col grid — same row. (3) Conclusion + Recommendation move INTO the anatomy grid (8 boxes share the same 2-col layout). (4) Clinical workstation refresh via `frontend-design` skill: warm ivory bg + white document surface, deep teal accent (`#0E3A47`) on the procedure-type toggle + signature stripe, serif page title (Georgia stack), small-caps section labels, mono date/duration values, warm coral Finalize action (`#C66B4D`), tinted card for the screenshots rail. |
| 2026-09-06 | report-editor-unify-patient-procedure-bullet-attached | (1) Patient row unified with Procedure — both now use the same 4-col / 3-col meta-row treatment (small-caps label above, value below; MRN/DOB use font-mono for the data feel). (2) Bullet button enhanced to a 28×28 icon-only round button (lucide `List` icon) anchored in the textarea's bottom-right with a teal hover state + tooltip. (3) Attached screenshot now reads as "chosen": teal border (`#0E3A47`) + soft teal-tint background + subtle shadow + small uppercase "ATTACHED" badge at the top-right corner so the doctor can scan a long timeline. The bottom-left toggle button recolored to teal to match. |
| 2026-09-06 | screenshot-tile-i18n-and-badge-polish | (1) Added `common.attached` to EN + AR (the previous ATTACHED badge rendered as raw dotted path `COMMON.ATTACHED`). (2) Replaced the wide ATTACHED pill with a compact 24×24 teal circle + `Check` icon from lucide — reads as a clean "chosen" mark. (3) Timestamp pill refined: compact center-pinned `bg-black/75 font-mono tabular-nums` so JPEG color tints don't bleed through (was `bg-black/50` + plain text). |
| 2026-09-06 | report-editor-procedure-center-print-regen-thumbnails | (1) Procedure-type toggle centered (`text-center` + `mx-auto`) under the procedure meta-row. (2) Print button actually prints — `PrintPreview` is now a forwardRef with imperative `print()` handle that awaits the iframe `load` event (4s timeout) then calls `iframe.contentWindow.print()` to trigger the OS print dialog directly. Falls back to OS viewer on failure. (3) Re-render PDF moved from the top-right header to the bottom of the document (next to Finalize) so the workflow reads fill → finalize → re-render → print. (4) Screenshot image `object-cover` → `object-contain` so the full frame is visible (letterboxed on tall/wide captures) instead of cropped. |
| 2026-09-06 | print-preview-and-input-unify | (1) Print preview fix — iframe was 1×1 px (Chromium couldn't render a preview → "This app doesn't support print preview"). Now full-viewport but invisible + non-interactive so the OS print dialog can preview the PDF. (2) Print button label `Print (opens PDF)` → `Print` (stale label from when it opened the viewer). (3) Re-render PDF moved into the screenshots rail as a teal-accented primary button — PDF controls + screenshots are now one cluster. (4) Unified input styles — extracted a `FIELD_CLASS` constant for select + input + textarea with consistent border / bg / padding / focus / hover. |
| 2026-09-06 | print-via-webcontents-save-changes-at-end | (1) Print preview now works — switched to Electron's `webContents.print()` via a new `REPORTS_PRINT` IPC channel. Main process loads the PDF in a hidden BrowserWindow with the Chromium PDF viewer and calls `webContents.print({ silent: false, printBackground: true, pageSize: 'Letter' })`. The iframe-based PrintPreview + `printIframeRef` are gone. The `REPORTS_PRINT` channel is added to `EXEMPT_CHANNELS`. (2) Dropped the `Empty falls back to clinic default: <value>` / `Overriding the clinic default for this report` hint line under the Pre-medication input — the input's `placeholder` already shows the fallback, the override state is implicit from the value. (3) Renamed "Re-render PDF" → "Save changes" (EN) / "حفظ التغييرات" (AR). Moved OUT of the screenshots rail to the very end of the page (outside the document, under the rail) as a teal-accented primary button. |
| 2026-09-06 | save-changes-vs-finalize-and-rail-max-height | (1) Bottom of the document shows one primary action at a time: no PDF yet → `Finalize report` (coral, creates the report row + locks procedure + generates first PDF); else → `Save changes` (teal, regenerates the PDF with edits). (2) Screenshots grid capped at `max-h-[60vh]` + `overflow-y-auto` so the rail stays inside the viewport on tall pages. |
| 2026-09-06 | pdf-min-bytes-print-preview-reveal-explorer | (1) `MIN_PDF_BYTES` lowered from 5_000 to 1_000 — minimal reports (no screenshots, empty boxes) legitimately produce ~2-3 KB PDFs; the 5 KB threshold was tuned for typical reports. 1 KB still catches a true truncated write while letting minimal reports through. Fixes `Error: PDF too small: 2794 bytes`. (2) Print preview fix attempt — the previous `show: false` (hidden) BrowserWindow couldn't render a preview because Chromium needs a compositor surface. Switched to `show: true` but positioned `x: -10000, y: -10000` + `minimized: true` + `skipTaskbar: true` so the doctor never sees the window. (3) Dropped Reveal in Explorer button + `handleRevealPdf` callback + `FolderOpen` icon import (the screenshot timeline already surfaces the source files). Updated the existing test that asserted the reveal button's behavior. |
| 2026-09-06 | finalize-idempotent | `reportsRepo.finalize` made idempotent on already-finalized rows (was throwing "not found or already finalized" — left the report stuck in `status='finalized', pdfPath=null` if the doctor clicked Back between `finalize` and `regenPdf`). Now: row missing → throw `IPC_NOT_FOUND`; row in 'finalized' state → no-op, return the current row (no `finalizedAt` bump so the original finalize event timestamp is preserved). The renderer's `handleFinalize` recovers correctly on the half-finalized state. |
| 2026-09-06 | remove-autosave | `useAutoSave` hook dropped from the editor — the doctor explicitly asked to drop auto-save since the page now has a Save changes button. Textarea / select / input / template-insert / bullet handlers now only call `setLocal` (in-memory React state); no IPC per change. `handleRegenPdf` (Save changes) flushes the current 8 box fields via `updateDraft` / `updateFinalized` BEFORE calling `regenPdf` so the regenerated PDF reflects the latest edits. The save-indicator `<p>` + `indicatorText` useMemo are gone. Replaced the previous autosave tests with three new contract tests: typing does NOT fire IPC, Save changes persists box edits + regenerates the PDF, finalized report routes to `updateFinalized`. `procedure_type` / `instrument` / `premedication_override` still persist immediately on each change via their dedicated IPC channels. |
