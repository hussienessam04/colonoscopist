...
> Canonical phase sign-off status. Updated by each plan's `docs(plan)` summary commit when its `phase_status` flips to `complete`.
...
- **TRIM-02 — multi-segment trim.** v1 ships single-window trim. v1.1 can support multi-segment trim per the deferred ideas.
- **REV-05 — video annotations.** Out of Phase 5 scope; requires an annotation overlay that overlays polygons on top of the `<video>`. Phase 6 PDF report will compose screenshots; in-video annotations are a separate addition.
- **Audio playback in review.** Phase 3 D-08 locked recording as `audio:false`; the review screen plays silent video per the phase constraint. Audio capture/playback is a separate concern that would change the recording ffmpeg flags.
- **Exact-cut re-encode trim.** v1 ships `-ss before -i -c copy` (D-08, ±500ms per PITFALLS §1). A v1.1 re-encode path (e.g. `-ss after -i -c:v libx264 -preset veryfast`) was deliberately deferred.
- **Browser seek-by-keyframe > 30 minutes.** Plan 04 caps Trim's Apply button at 30 minutes via `MAX_TRIM_DURATION_MS = 30 * 60 * 1000` to avoid seek drift; v1.1 can lift the cap with the re-encode path above.
...
`.planning/phases/05-screenshots-procedure-review-trim/05-UAT.md` — Windows hardware smoke checklist (real USB capture card, real workstation). Marked `pending` until the doctor runs it on a real workstation. **Required for `phase_status: complete` per Plan 04 task acceptance.**

---

## Phase 4 — Recording + Timer + Device-Lost

Status: complete (closed earlier in phase). Not re-verified here.

## Phase 3 — Capture Enumeration + Live Preview

Status: complete (closed earlier in phase).
